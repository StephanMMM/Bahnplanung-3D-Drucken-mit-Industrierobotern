...
namespace Studienprojekt
{
    class Infill
    {
        ...
        /*Old one:works for 50% but is slightlie tilted
        public int[,,] Generate_3DInfill() {
            int[,,] Sample = new int[2 * infill_density - 1 , infill_density - 1, 2 * infill_density - 1 ];
            //Definition der Einzel Zelle
            //Bottom Half
            for (int depth = 0; depth < infill_density / 2; ++depth)
            {
                for (int width = infill_density / 2 + depth; width < infill_density + infill_density / 2 - depth; ++width)
                {
            ...
            for (int height = 0; height < infill_density / 2; ++height)
            {
                for (int depth = 0; depth < infill_density / 2 - height; ++depth)
                {
                    ...
                    Sample[0, depth, infill_density - height] = 1;//left side standing triangle 
                }
                for (int width = infill_density - height; width < infill_density + height ; ++width)
                {
                    Sample[width, infill_density - 2, infill_density / 2 + height] = 1;//line
                }
                for (int depth = 0; depth < infill_density / 2 + height; ++depth)//faces
                {
                    Sample[infill_density / 2 - height + depth, depth, height + 1] = 1;
                    Sample[infill_density + infill_density / 2 + height - 1 - depth, depth, height + 1] = 1;//lower half

                    Sample[infill_density / 2 + height - depth - 1, infill_density - depth - 2, infill_density - height] = 1;
                    if (depth != infill_density / 2 + height - 1)
                    {
                        Sample[infill_density + infill_density / 2 - height + depth, infill_density - depth - 2, infill_density - height] = 1;//upper half
                    }
                }
            }
            for (int i = 0; i < infill_density; i++)//mirror the lower half to the upper half
            {
                for (int y = 0; y < infill_density - 1; ++y)
                {
                    for (int x = 0; x < 2 * infill_density - 1; ++x)
                    {
                        Sample[x, y, 2 * infill_density - 2 - i] = Sample[x, y, i + 1];
                    }
                }
            }
            for (int width = 0; width < infill_density / 2; ++width)
            {
                ...
                {
                    Sample[width, infill_density-2-depth , infill_density] = 1;//Mid flat Triangle
                    if(width != 0)
                    {
        ...
        private int Is_3DInfill(int x, int y, int z)
            ...
            x = x % (2*infill_density-1);
            z = z % (2*infill_density-1);
            if (!isEven)
            {
            x += (infill_density-1);
            z += (infill_density-1);
        ...
        */
        ...
        {
            //the size a of one edge for a given Infill density x for 3dInfill = 100/Infilldensity in %
            //would be a ~~ x+1(a-1+a^-2+ O(a^-3))
            //even as these are streched (length is measured in voxels not eucledian distance) to accomodate the 45° angle of the voxels they still very much fit into that degree of accuracy for /50% ~5% /33% == 25% ~4%/ 20% ~ 1,8% / 10% ~0,7% inaccurate
            //testing
            Infill infil = new Infill();
            infil.BaseCell = infil.Generate_3DInfill();
            Console.Write(infil.infill_density);
            int[,,] muster = new int[33, 33, 80];
            using (StreamWriter outputFile = new StreamWriter(path: @"D:\MyStuff\Studium\Studienprojekt\C#\InfillDebug.dat"))
            {
                ...
                for (int z = 0; z < 80; ++z)
                {
                    for (int y = 0; y < 33; ++y)
                    {
                        for (int x = 0; x < 33; ++x)
                        {
                            muster[x, y, z] = infil.Is_3DInfill(x, y, z);
                            if (muster[x, y, z] == 1)
                            {
                                outputFile.Write(x + " " + y + " " + z + "\r\n");
                            }
